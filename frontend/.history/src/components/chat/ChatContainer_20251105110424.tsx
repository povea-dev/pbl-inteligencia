...
export const ChatContainer: React.FC = () => {
  ...
  const handleSendMessage = async (content: string) => {
    ...
    try {
      ...
      // 2. Actualizar actividad de la sesión
      await firebaseService.updateSessionActivity(currentSession.id);

      // 3. Si el backend está disponible, obtener feedback de IA
      if (backendAvailable) {
        try {
          ...
          // 4. Guardar respuesta de la IA en Firebase
          await firebaseService.saveMessage({
            sessionId: currentSession.id,
            role: 'assistant',
          ...
          // Mensaje de fallback si la IA no responde
          await firebaseService.saveMessage({
            sessionId: currentSession.id,
            role: 'assistant',
            content: '⚠️ El servicio de IA no está disponible en este momento. Tu respuesta ha sido guardada y será procesada más tarde.'
          });
        }
      } else {
        // Backend no disponible - mensaje informativo
        await firebaseService.saveMessage({
          sessionId: currentSession.id,
          role: 'assistant',
          content: '⚠️ El backend no está conectado. Tu respuesta ha sido guardada. Por ahora puedes continuar escribiendo y todo quedará registrado.'
        });
      }
          ...
          {backendAvailable !== null && (
            <div className="ml-4">
              <div className={`flex items-center gap-2 text-xs px-3 py-1 rounded-full ${
                ...
                {backendAvailable ? 'IA Conectada' : 'Sin IA (solo registro)'}
              </div>
            </div>
          )}
        </div>
...
Deberías tener estos componentes creados:
```
src/components/chat/
├── MessageBubble.tsx ✅
├── MessageList.tsx ✅
